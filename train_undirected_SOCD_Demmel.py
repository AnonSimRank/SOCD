...
from SOCD_Demmel import SOCD_Demmel
import warnings
...
obj = SOCD_Demmel(laplacian_mat, init_vec, community_k, iter_num)
#converge_Ritz_pair_dict, orth_mat_Q = obj.selective_orth_full()
k_means_ = obj.k_means()
t2 = time.perf_counter()
print("requires time:%s" % ((t2 - t1) * 1000))
...
# save train labels ##############################################################
train_labels_file_name = r"\{}_SOCD_Demmel_train_labels_5.txt".format(dataset_name)
...
metric_score_txt = r"\{}_SOCD_Demmel_metric_score_5.txt".format(dataset_name)
